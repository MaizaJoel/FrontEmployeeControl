...
const Dashboard = () => {
            ...
            <Col md={4}>
                <Card className="h-100 shadow-sm border-0 border-start border-4 border-warning">
                    <Card.Body>
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted text-uppercase mb-2">Solicitudes Globales</h6>
    ...
    const renderEmployeeView = () => (
        <Row className="g-4">
            {/* Tarjeta 1: Mi Estado Hoy */}
            <Col md={6}>
                <Card className={`h-100 shadow-sm border-0 border-start border-4 ${summary?.ultimoFichaje?.includes('ENTRADA') ? 'border-success' : 'border-secondary'}`}>
                    <Card.Body>
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted text-uppercase mb-2">Mi Estado Hoy</h6>
                                <h3 className="fw-bold text-dark mb-0">{summary?.ultimoFichaje || 'Sin registro'}</h3>
                            </div>
                            <div className={summary?.ultimoFichaje?.includes('ENTRADA') ? 'text-success' : 'text-secondary opacity-50'}>
                                <i className="bi bi-clock-history fs-1"></i>
                            </div>
                        </div>
                        <small className="text-muted">Último evento registrado</small>
                    </Card.Body>
                </Card>
            </Col>
            ...
            <Col md={6}>
                <Card className="h-100 shadow-sm border-0 border-start border-4 border-info">
                    <Card.Body>
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 className="text-muted text-uppercase mb-2">Mis Solicitudes</h6>
                                <h2 className="display-4 fw-bold text-dark mb-0">{summary?.misSolicitudesPendientes || 0}</h2>
                            </div>
                            <div className="text-info opacity-50">
                                <i className="bi bi-file-earmark-text fs-1"></i>
                            </div>
                        </div>
                        <small className="text-muted">Adelantos pendientes de aprobación</small>
                    </Card.Body>
                </Card>
            </Col>
        </Row>
    ...
    return (
        <div className="container mt-4">
            <h2 className="mb-4 fw-bold text-secondary">
                {summary?.isAdminView ? 'Panel de Control (Global)' : 'Mi Resumen Personal'}
            </h2>
            {summary?.isAdminView ? renderAdminView() : renderEmployeeView()}
        </div>
    );
};
