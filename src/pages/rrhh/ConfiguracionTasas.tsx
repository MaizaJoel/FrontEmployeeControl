...
const ConfiguracionTasas = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [tasasData, horariosData] = await Promise.all([
    ...
    const handleEdit = (horario: HorarioTasa) => {
        ...
        setSelectedDays(horario.diasAplicacion.split(','));
                            ...
                            <Form onSubmit={handleSave}>
                                ...
                                <div className="d-grid gap-2">
                                    <Button variant={editingId ? 'warning' : 'success'} type="submit" disabled={loading}>
                                        {loading ? <Spinner size="sm" animation="border" /> : (editingId ? 'Actualizar Regla' : 'Guardar Regla')}
                                    </Button>
                                    {editingId && (
